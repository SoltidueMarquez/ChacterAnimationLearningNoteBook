...
def patchfunc(P, Xp, hscale=3.937007874, vscale=3.0):
    ...
    在地形高度图上采样高度值（双线性插值）
    参数:
        P: 地形高度图（patches），是一个三维数组。根据上下文，P的Shape为 (nsamples, H, W)，
            其中nsamples是地形块的数量，H和W是地形块的高度和宽度（以像素为单位）。
        Xp: 采样二维坐标点，表示在世界空间中的位置（x, z）。注意：这个函数处理多个点，所以Xp是一个二维数组，形状为 (N, 2)，其中N是点的数量。
        hscale: 水平缩放因子，用于将世界坐标转换为地形图上的像素坐标。（默认值 3.937 ≈ 英寸转厘米）
        vscale: 垂直缩放因子，用于调整高度的缩放。（默认值 3.0）
    功能：
        这个函数的目的是根据给定的二维坐标（Xp）在高度图（P）上采样对应的高度值。
        核心功能是根据输入的 2D 世界坐标 Xp，在高度图 P 上执行双线性插值获取对应位置的高度值，并应用垂直缩放。
    与论文的对应关系
        在论文中（第 4.2 节 Terrain Fitting）：
        "Our system can automatically produce appropriate and expressive locomotion for a character moving over rough terrain" (Fig. 1)
        高度图采样：通过射线追踪从游戏场景中提取高度图（patches），用于拟合地形。
        坐标转换：将运动数据中的脚部接触点坐标转换为高度图上的位置（需考虑分辨率）。
        双线性插值：解决高度图离散采样问题，平滑获取连续高度值（公式 (1)-(4) 的拟合误差计算依赖此插值）。
        垂直缩放：调整高度值以适应实际场景比例（vscale 参数）。
    """
    # 步骤1: 坐标转换（世界坐标 → 像素坐标 + 中心偏移）
    # 坐标转换与归一化：将世界坐标 Xp 除以 hscale（转换为像素单位），并平移至高度图中心。
    # 将世界坐标映射到高度图像素坐标系（原点在中心）。
    # np.array创建多维数组；整除运算符 //，P.shape[1]是高度图的行数，P.shape[2]是高度图的列数，整除2是为了寻找到其中心点，
    # 例如，若 P 形状为 (100, 256, 256)（100 个地形块，每个块 256x256 像素），则最终得到：np.array([128, 128])（假设高度图尺寸为 256x256）。
    Xp = Xp / hscale + np.array([P.shape[1]//2, P.shape[2]//2])

    # 步骤2: 计算双线性插值权重
    A = np.fmod(Xp, 1.0)  # 取余操作提取小数部分 (α_x, α_z)
    # 左下角整数坐标；
    # np.clip(..., 0, boundaries)将索引限制在高度图的有效范围内：
    # np.floor(Xp)取浮点坐标的向下取整整数；.astype类型转换
    X0 = np.clip(np.floor(Xp).astype(np.int), 0, np.array([P.shape[1]-1, P.shape[2]-1])
    # 右上角整数坐标；np.ceil(Xp)取浮点坐标的向上取整整数；
    X1 = np.clip(np.ceil (Xp).astype(np.int), 0, np.array([P.shape[1]-1, P.shape[2]-1]))

    # 步骤3: 采样四个相邻像素高度，此计算直接支持论文中的地形误差计算（见公式2），以H0为例
    # 第一维度 :选择所有地形块（保留N个地形块维度）
    # 第二维度 X0[:,0]取所有采样点的行索引（左下角Y坐标）
    # 第三维度 X0[:,1]取所有采样点的列索引（左下角X坐标）
    # 输出结果：获得每个采样点在所有地形块上的左下角高度
    H0 = P[:, X0[:, 0], X0[:, 1]]  # 左下点高度
    H1 = P[:, X0[:, 0], X1[:, 1]]  # 右下点高度
    H2 = P[:, X1[:, 0], X0[:, 1]]  # 左上点高度
    H3 = P[:, X1[:, 0], X1[:, 1]]  # 右上点高度

    # 步骤4: 双线性插值
    # A[:,0]是采样点在X方向小数偏移量
    HL = (1 - A[:, 0]) * H0 + A[:, 0] * H2  # 水平插值（左）
    HR = (1 - A[:, 0]) * H1 + A[:, 0] * H3  # 水平插值（右）
    
    # Y方向插值并缩放
    # 使用X方向偏移量A[:,0]作为混合权重：左顶点权重为1-A[:,1]，右顶点权重为A[:,1]，再乘以地形缩放指数。
    # array[..., np.newaxis]表示在数组的末尾增加一个新维度。(vscale * ...)是生成一个形状为(num_patches, num_points)的2D数组。[..., np.newaxis]将其转换为形状(num_patches, num_points, 1)的3D数组。
    # 在PFNN中确保：
    # 1.地形高度数据与关节位置维度兼容
    # 2.支持大规模地形数据库的矢量化计算
    # 3.保障论文Section 4.2地形拟合算法的数学严谨性
